...
function AxeUltiF() {
	...
	var HEnts = Game.PlayersHeroEnts().map(function(ent) {
		...
		return Entities.IsAlive(ent) && !(Entities.IsBuilding(ent) || Entities.IsInvulnerable(ent)) && Entities.IsEnemy(ent) && Entities.GetRangeToUnit(MyEnt, ent) <= Abilities.GetCastRangeFix(Ulti)
